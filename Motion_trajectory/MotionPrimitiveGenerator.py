...
class MotionPrimitiveGenerator:
    ...
    def generate_motion_primitives(cls):
        ...
        for idx, (a, sa_rate) in enumerate(itertools.product(list_samples_a, list_samples_sa_rate)):
    ...
    def create_mirrored_primitive(cls, primitive):
        ...
        try:
            ...
            if hasattr(primitive, 'primitive_id'):
                mirrored_traj.primitive_id = f"{primitive.primitive_id}_mirrored"
            else:
                mirrored_traj.primitive_id = "unknown_mirrored"
    ...
    def generate_sample_trajectories(cls, list_primitives,
        ...
        if initial_pose is not None:
            current_pose = initial_pose  # 使用传入的初始姿态
        else:
            current_pose = (np.array([0.0, 0.0]), 0.0)  # 默认姿态
