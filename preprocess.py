...
class TrajectoryProcessor:
    ...
    def process_csv(self, file_path):
        ...
        # Smooth velocity and position data
        df['smoothed_velocity'] = df['velocity'].rolling(window=5, center=True).mean()
        df['smoothed_position_x'] = df['position_x'].rolling(window=5, center=True).mean()
        df['smoothed_position_y'] = df['position_y'].rolling(window=5, center=True).mean()

        df['scaled_position_y'] = df['smoothed_position_y']

        # Calculate lateral and longitudinal acceleration
        df['lateral_acceleration'] = df['smoothed_position_x'].diff() / df['time_step'].diff()
        df['longitudinal_acceleration'] = df['scaled_position_y'].diff() / df['time_step'].diff()

        # Determine position based on y-axis values
        y_min = df['scaled_position_y'].min()
        y_max = df['scaled_position_y'].max()
        if y_min < -0:
            df['position'] = 3
        elif y_max > 6:
            df['position'] = 3
        ...
        df['ttc_static1'] = df.apply(lambda row: calculate_ttc(row['smoothed_position_x'], row['scaled_position_y'], row['smoothed_velocity'], static_position1[0], static_position1[1]), axis=1)
        df['ttc_static2'] = df.apply(lambda row: calculate_ttc(row['smoothed_position_x'], row['scaled_position_y'], row['smoothed_velocity'], static_position2[0], static_position2[1]), axis=1)
    ...
    def process_and_score_file(self, file_path):
        ...
        if 'collision' in df.columns and (df['collision'] == 1).any():
            ...
            ttc_breakpoints = [self.ttc_p25, self.ttc_p50, self.ttc_p75]
            ttc_scores = [25, 50, 75]
            ttc_score = self.calculate_ttc_score(min_ttc, ttc_breakpoints, ttc_scores)

        lateral_breakpoints = [self.lateral_p25, self.lateral_p50, self.lateral_p75]
        lateral_scores = [75, 50, 0]

        longitudinal_breakpoints = [self.longitudinal_p25, self.longitudinal_p50, self.longitudinal_p75]
        longitudinal_scores = [75, 50, 0]
